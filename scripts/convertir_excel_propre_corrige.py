...
print("\n" + "="*70)
print("🔄 CONVERSION CORRIGÉE DU FICHIER EXCEL")
print("="*70)

# Charger le fichier Excel brut
df_brut = pd.read_excel("exportateurs-word.xlsx", header=None)
...
for index, row in df_brut.iterrows():
    texte = str(row[0]) if pd.notna(row[0]) else ""
    
    if len(texte) < 10 or texte.startswith("Page") or "INDEX" in texte.upper():
        continue
